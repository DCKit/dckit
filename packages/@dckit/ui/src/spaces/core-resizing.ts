...
export function createResize(store: ISpaceStore) {
  ...
  return {
    startResize<
      T extends SyntheticEvent<HTMLElement> | MouseEvent | TouchEvent
    >(
      resizeType: ResizeType,
      e: T,
      space: ISpaceDefinition,
      ...
      const size =
        space.orientation === Orientation.Horizontal ? rect.width : rect.height
      const originalCoords = getCoords(e)
      const startSize = targetSize.resized
      const minimumAdjust =
        coalesce(space.minimumSize, 20)! - size + targetSize.resized
      const maximumAdjust = space.maximumSize
        ? space.maximumSize - size + targetSize.resized
        : undefined
      ...
      const withPreventDefault = (e: T) => {
        ...
        throttle(
          (x, y) => window.requestAnimationFrame(() => resize(x, y)),
          RESIZE_THROTTLE
        )(lastX, lastY)
      }
      ...
      const removeListener = () => {
        ...
        window.removeEventListener(
          moveEvent,
          withPreventDefault as EventListener
        )

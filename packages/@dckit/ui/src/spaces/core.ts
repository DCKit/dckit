...
function getOrientation(anchor: AnchorType | undefined) {
  return anchor === AnchorType.Bottom || anchor === AnchorType.Top
...
export function createStore(): ISpaceStore {
    ...
    for (var i = 0, len = parent.children.length; i < len; i++) {
      ...
      if (space.type === Type.Fill) {
        anchorUpdates(space).forEach((info) => {
          ...
          const anchoredSpaces = parent.anchoredChildren(
            info.anchor,
            space.zIndex
          )
  ...
  const store: ISpaceStore = {
    ...
    removeSpace: (space) => {
      ...
      if (space.parentId) {
        const parentSpace = getSpace(space.parentId)
        if (parentSpace) {
          parentSpace.children = parentSpace.children.filter(
            (s) => s.id !== space.id
          )
          recalcSpaces(parentSpace)
        }
    ...
    updateSpace: (space, props) => {
      ...
      if (
        coalesce(space.centerContent, CenterType.None) !==
        coalesce(centerContent, CenterType.None)
      ) {
  ...
  store.createSpace = (
    ...
    newSpace.anchoredChildren = (anchor, zIndex) =>
      newSpace.children.filter(
        (s) =>
          s.type === Type.Anchored && s.anchor === anchor && s.zIndex === zIndex
      )
